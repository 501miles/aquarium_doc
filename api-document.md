...
| room_device_uuid | string | N | 室内环境温度计序列号 |
...
| room_device_uuid | string | 室内环境温度计序列号 |
...
| last_temp | float | 最近一次上报的温度 |
| room_last_temp | float | 最近一次上报的室内环境温度 |
...
| width | int | N | 宽度(mm) |
| height | int | N | 高度(mm) |
| rack_material_id | int | N | 钢架材质id |
| skeleton_type_id | int | N | 骨架类型id |
...
| pvc_usage | int | N | pvc用途枚举：0->鱼缸垫板，1->底缸垫板 |
...
| width | int | 宽度(mm) |
| height | int | 高度(mm) |
| rack_material_id | int | 钢架材质id |
| rack_material_name | int | 钢架材质名称 |
| skeleton_type_id | int | 骨架类型id |
| skeleton_type_name | int | 骨架类型名称 |
| beam_count | int | 横梁数量 |
| carling_count | int | 纵梁数量 |
| surface_color | int | 表面颜色枚举：0->砂纹黑，1->砂纹白 |
| need_pvc_bottom_board | bool | 是否需要pvc底板 |
| pvc_usage | int | pvc用途枚举：0->鱼缸垫板，1->底缸垫板 |
